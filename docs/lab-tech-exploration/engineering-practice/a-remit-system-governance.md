...
这个系统也不是高并发系统。实际观察到的峰值 QPS 只有个位数，但它承载的是真实资金，而且单笔和总体金额都不小。
...
如果系统没有独立的附加费用能力，就只能通过内部人工方式调整订单金额。
...
PageHelper 的 `Page` 对象本身又同时带有分页和 List 的语义，相关列表访问甚至可能返回对象自身，使分页结果对象和列表概念进一步混合。
...
### 案例一：分布式流程不能假设严格顺序和 exactly once
