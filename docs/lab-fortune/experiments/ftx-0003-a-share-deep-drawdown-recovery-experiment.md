...
例如某 ETF 的 Anchor 为 4.00 元，当前价格为 3.00 元，则：

$$

\frac{3.00-4.00}{4.00}=-25\%

$$

即该 ETF 当前处于相对 Anchor 回撤 25% 的位置。

Anchor 一旦在实验启动时确认并冻结，本轮实验期间不再因为之后出现新的阶段高点而滚动调整。

因此：

- Anchor 是执行坐标，不是对市场顶部的预测；
- 各 ETF 使用各自独立的 Anchor；
- 后续所有 -20%、-25%、-30% 等 Level，均相对于该标的固定 Anchor 计算；
- Anchor 不因为市场反弹、新高、账户盈亏或新增资金而调整；
- 如果 Anchor 的确定规则在实验启动前发生修改，应记录在参数设计文档中；
- 实验启动并冻结参数以后，不再根据后续行情反向修改 Anchor。

> **固定 Anchor 的目的，是让整个实验始终使用同一套执行坐标，避免市场运行过程中不断移动基准，导致原有 Level 的含义发生变化。**
