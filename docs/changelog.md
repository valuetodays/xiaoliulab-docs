...
- 新增快捷入口 [技术实践索引](/engineering/)，集中提供遗留系统治理、渠道集成、测试、数据访问、容器网络与日志排查等工程实践文章的阅读入口。

## 2026-09-10

### ✨ 新增

- 新增 [一个留学缴费遗留系统的治理：边界、验证与工程判断](/lab-tech-exploration/engineering-practice/a-remit-system-governance)，记录一个低并发、高金额、长流程、多人工节点的 Java 遗留系统如何逐步治理重复代码、隐式依赖、技术栈叠加、渠道边界和测试缺失，并从影响范围、可逆性、可验证性、维护成本增速与实际收益等维度总结工程判断。
...
- 新增 [从 Spring Boot 迁移到 Quarkus：一份面向真实业务的基础能力验证清单](/lab-tech-exploration/application-frameworks/spring-boot-to-quarkus-capability-validation)，记录一次因 Spring Boot 启动速度和资源占用而展开的技术选型，在尝试多种轻量 Java Web 框架及 Go 后，最终选择在轻量化与业务基础能力之间更平衡的 Quarkus，并在迁移业务前系统验证数据访问、REST、Bean、配置、事务、校验、定时任务、异常处理、日志、监控、部署等能力，同时记录从 JPA/Panache 到继续使用 MyBatis-Plus 的数据访问演进。
...
- 新增 [一次 User-Agent 解析引发的内存与登录性能问题：从 YAUAA 缓存怀疑到删除无用功能](/lab-tech-exploration/application-frameworks/yauaa-user-agent-memory-login-performance)，记录旧项目 Heap 长时间维持高位与登录接口额外耗时约 1 秒的排查过程，通过 heap dump、缓存对照实验及源码分析定位历史遗留的 User-Agent 解析逻辑，并移除当前业务不再使用的相关代码与依赖。
- 新增 [PageHelper 两种分页写法对比：一次泛型失真的排查与源码分析](/lab-tech-exploration/application-frameworks/pagehelper-pagination-generic-type-safety)，对比 `doSelectPageInfo` 与显式接收 `List<T>` 后构造 `PageInfo` 的分页写法，从源码关系解释泛型失真的原因，并给出让方法签名、Mapper 返回类型与实际数据保持一致的维护方式。
